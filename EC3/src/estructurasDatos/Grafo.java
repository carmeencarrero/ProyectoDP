...
public class Grafo {
	 ...
	 * @return No retorna ningun valor
	 */
	public void adyacentes(int origen, LinkedList<Integer> ady) {
	...
	/**
	 * Método que calcula el camino mínimo entre dos nodos (menos valor tiene)
	 * 
	 * @param origen
	 * @param destino
	 * @return camino lista con los nodos
	 */
	...
	/**
	 * Método que devuelve el nodo que más caminos tiene
	 * 
	 * @return nodoMax devuelve el nodo del que salen mas caminos
	 */
	public int nodoMasCaminos() {
		int nodo = 0;
		int nodoMax = 0;
		int caminosNodo;
		int caminosMax = 0;
		...
		while (nodo < numNodos) {
			caminosNodo = 0;
			for (int i = 0; i < numNodos; i++) {
				if (arcos[nodo][i] != 0 && arcos[nodo][i] != Grafo.INFINITO) {
					caminosNodo++;
				}
			}

			if (caminosNodo > caminosMax) {
				caminosMax = caminosNodo;
				nodoMax = nodo;
			}
			nodo++;
		}
	...
	/**
	 * Metodo publico que devuelve todos los caminos existentes entre el vertice
	 * y el nodoFinal en rutas
	 * 
	 * @param vertice
	 * @param nodoFinal
	 * @param rutas
	 */
	public void recorridoProfundidad(int vertice, int nodoFinal,
			LinkedList<LinkedList<Integer>> rutas) {

		LinkedList<Integer> verticesVisitados = new LinkedList<Integer>();
		LinkedList<Integer> camino = new LinkedList<Integer>();
		profundidad(this, camino, rutas, vertice, nodoFinal, verticesVisitados);
	}

	/**
	 * Metodo privado que devuelve todos los caminos existentes entre el vertice
	 * y el nodoFinal en rutas
	 * 
	 * @param graph
	 * @param camino
	 ...
	 */
	private void profundidad(Grafo graph, LinkedList<Integer> camino,
			LinkedList<LinkedList<Integer>> rutas, Integer vertice,
			Integer nodoFinal, LinkedList<Integer> verticesVisitados) {
		LinkedList<Integer> verticesAdy = new LinkedList<Integer>();
		...
		if (vertice.equals(nodoFinal)) {
			@SuppressWarnings("unchecked")
			LinkedList<Integer> caminoOK = (LinkedList<Integer>) camino.clone();
			rutas.add(caminoOK);
			...
			graph.adyacentes(vertice, verticesAdy);
			while (!verticesAdy.isEmpty()) {
				vertice2 = verticesAdy.getFirst();
				verticesAdy.remove(vertice2);
				if (!verticesVisitados.contains(vertice2)) {
					profundidad(graph, camino, rutas, vertice2, nodoFinal,
							verticesVisitados);
				}
			}
	...
	/**
	 * Metodo que devuelve el camino cogiendo los adyacentes con los numeros mas pequeños entre una sala y otra
	 * 
	 * @param vertice
	 * @param nodoFinal
	 * @param ruta
	 */
	public void caminoSalaMinima(Integer vertice, Integer nodoFinal,
			LinkedList<Integer> ruta) {
		LinkedList<Integer> verticesVisitados = new LinkedList<Integer>();
		caminoSalaMinima(this, vertice, nodoFinal, verticesVisitados, ruta);
	}

	/**
	 * 
	 * Metodo privado que devuelve el camino cogiendo los adyacentes con los numeros mas pequeños entre una sala y otra
	 * 
	 * @param graph
	 * @param vertice
	 * @param nodoFinal
	 ...
	 * @param ruta
	 */
	private void caminoSalaMinima(Grafo graph, Integer vertice,
			Integer nodoFinal, LinkedList<Integer> verticesVisitados,
			LinkedList<Integer> ruta) {
		LinkedList<Integer> verticesAdy = new LinkedList<Integer>();
		Integer vertice2;
		...
		if (vertice.equals(nodoFinal)) {
			System.out.println("El camino ha llegado al final");
		} else {
			graph.adyacentes(vertice, verticesAdy);
		
				while (!verticesAdy.isEmpty() && !ruta.contains(nodoFinal)) {
			
					vertice2 = verticesAdy.getFirst();
					verticesAdy.remove(vertice2); // integer por el remove
					if (!verticesVisitados.contains(vertice2)) {
						caminoSalaMinima(graph, vertice2, nodoFinal,
								verticesVisitados,ruta);
					}
				
				}
				if (!ruta.getLast().equals(nodoFinal))
				ruta.remove(vertice);
		}
	}
	...
	public static void main(String[] args) {
		...
		LinkedList<Integer> camino = g.caminoMinimo(0, 9);
		System.out.println("Camino mínimo entre 0 y 9");
		for (int i = 0; i < camino.size(); i++) {
			System.out.println(camino.get(i));
		}

		System.out.println("El nodo que tiene más caminos es: "
				+ g.nodoMasCaminos());
		LinkedList<LinkedList<Integer>> caminoss = new LinkedList<LinkedList<Integer>>();
		LinkedList<Integer> ruta = new LinkedList<Integer>();
		g.caminoSalaMinima(0, 9, ruta);

		 g.recorridoProfundidad(0, 9, caminoss);
//		 for (int i = 0; i < caminoss.size(); i++) {
//		 LinkedList<Integer> caminoos = caminoss.get(i);
//		 for (int j = 0; j < caminoos.size(); j++) {
//		 System.out.println("camino nuevo" + caminoos.get(j));
//		 }
//		 }
